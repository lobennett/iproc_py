"""discover — Scan a BIDS dataset and produce an editable YAML manifest for
iProc configuration generation.

Usage:
    bids_discover /path/to/bids_root \
...
Review it, edit T1 selections or exclude sessions, then pass to generate.
...
def _plain(v: Any) -> Any:
    ...
    generate) cannot construct, crashing the discover -> generate
...
def discover_subject(
    ...
    for ses_label, ses_query in session_items:
        ...
        # Per-session fieldmap regime (preptool vocabulary), so generate
    ...
    # confidence so generate can gate low-confidence input behind --force.
    detections = []
    fmap_type = None
    confidence = "high"
    for ses_label in sorted(sessions.keys(), key=_num_key):
